classdef AgentLinear < handle
   ...
   methods (Access = public)
       ...
       function obj = Agent(id, pos, goal, s, p_s, r, n_p, n_p_s, n_d, n_a, c, o_v, n) %It is a constructor, creates objects and assigns value to the properties and returns initialized object
           if nargin>0
               for i = 1:n
...
%      function that sets the preferred velocity of a specific agent
       ...
       function [avoiding, a_net, dmin] = FindVelocity(obj,others,t_iter,n,t_run,R)
...
%                     t_iter = 0.6;
                    obj.NewVelocity(1) = obj.PrefVelocity(1) - exp(-t_iter/delta)*(obj.PrefVelocity(1)-obj.NewVelocity(1)); %updating the new velocity of the agent
                    obj.NewVelocity(2) = obj.PrefVelocity(2) - exp(-t_iter/delta)*(obj.PrefVelocity(2)-obj.NewVelocity(2));
...
%                     a_i_x =  a_i_y,a_j_x,a_j_y
                    [desVagent_x, desVagent_y] = PVO_controller(t_run,delta, obj, others, tag, pose,t_iter,myMap_occupancy,occ_index, obs_start_pos_occ_grid ,grid_size,resol,dia_agent,tH,robo_origin,brake_flag,a_i_x,a_i_y,a_j_x,a_j_y);
...
%                     disp(strcat('Agent-',int2str(obj.Identity),'desired velocity-->', string(desVagent_x),':', string(desVagent_y)));
                    ...
                    obj.NewVelocity(1) = obj.OldVelocity(1) + a_x_r*t_iter; %updating the new velocity of the agent
                    obj.NewVelocity(2) = obj.OldVelocity(1) + a_y_r*t_iter;
