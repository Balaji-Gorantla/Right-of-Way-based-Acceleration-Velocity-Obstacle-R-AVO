...
%                     t_iter = 0.6;
                    ...
                    [obj.NewPosition(1), obj.NewPosition(2)] = pos_updater_agent(delta,obj,obj.NewVelocity(1),obj.NewVelocity(2),t_i);
                    obj.NewVelocity(1) = obj.PrefVelocity(1) - exp(-t_i/delta)*(obj.PrefVelocity(1)-obj.OldVelocity(1)); %updating the new velocity of the agent
                    obj.NewVelocity(2) = obj.PrefVelocity(2) - exp(-t_i/delta)*(obj.PrefVelocity(2)-obj.OldVelocity(2));
...
% 
                   ...
                   [desVagent_x, desVagent_y] = avo_safe_velocity(tag,obj,others,delta,t_i,tH,R_D);
                   [desVagent_x_temp, desVagent_y_temp] = avo_safe_velocity_new(tag,obj,others,delta,t_iter,tH,R_D);
...
%                     disp(strcat('Agent-',int2str(obj.Identity),'desired velocity-->', string(desVagent_x),':', string(desVagent_y)));
                    obj.OldVelocity = obj.NewVelocity;
                    [obj.NewPosition(1), obj.NewPosition(2)] = pos_updater_agent(delta,obj,desVagent_x, desVagent_y, t_i); %updating the new positions of the agent
                    obj.NewVelocity(1) = desVagent_x - exp(-t_i/delta)*(desVagent_x-obj.OldVelocity(1)); %updating the new velocity of the agent
                    obj.NewVelocity(2) = desVagent_y - exp(-t_i/delta)*(desVagent_y-obj.OldVelocity(2));
...
%                     hold off;
                    avoiding = 1;
                    if is_test_req
                        if obj.Identity == 2 || obj.Identity == 1
                            hold on;
                            plot(desVagent_x+obj.Position(1),desVagent_y+obj.Position(2),'r+');
                            hold on;
                            plot(obj.NewVelocity(1)+obj.Position(1),obj.NewVelocity(2)+obj.Position(2),'g+');
%                             hold on;
%                             plot(obj.NewPosition(1),obj.NewPosition(2),'k*','MarkerSize',10);
                            hold on;
                            for k=length(tag)
                                if norm(obj.Position - others(tag(k)).Position) < 2*(obj.Radius)
                                    col_dist = (norm(obj.Position - others(tag(k)).Position))/2;
                                    plot(col_dist+obj.Position(1),col_dist+obj.Position(2), 'r*', 'MarkerSize',20); 
                                end
                            end
                            hold off;
